...
{
  std::cerr
      << "Usage: " << argv0 << " [options] [directory]\n\n"
      << "Find files using the dirtoo filter language (not POSIX find primaries).\n"
      << "Default directory is the current working directory.\n"
      << "Also installed as dt-search (same binary behaviour).\n\n"
      << "Options:\n"
      << "  -f, --filter EXPR     Filter expression (default: match all)\n"
...
int main(int argc, char** argv)
{
  ...
  if (stats.errors > 0) {
    std::cerr << "warning: " << stats.errors << " path(s) could not be read\n";
  }
  return 0;
}
